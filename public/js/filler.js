                        ...
                        <div class="text-center py-4">
                            <div class="row cd-flex align-items-center justify-content-center mt-2">
                                <div class="col-1 mb-1 mt-1">
                                    <i class="fa-2x ${data.details[pi].isOwner ? "ri-vip-crown-2-fill" : "ri-user-fill"}"></i>
                                </div >
                                <div class="col-3">
                                    <h5>${data.shopname}</h5>
                                </div>
                                ...
                                <div class="col-2 ${data.details[pi].paid ? "bg-paid" : "bg-unpaid"}">
                                    <h5 class="bold-text">${data.details[pi].paid ? "PAID" : "UNPAID"}</h5>
                                </div>
                            ...
                            <div class="col-12 cd-flex align-items-center justify-content-center mt-2" id="showMore${i}" hidden>
                                ${generateReceiptsDetailsHelper(data, pi)}
                            </div>
                        </div >
                    </div >
                </div >
...
function generateReceiptsDetailsHelper(data, pi) {
    str = "";
    for (let i = 0; i < data.details.length; i++) {
        if (pi != i) {
            str += `<div class="row cd-flex align-items-center justify-content-center mt-2">
                        <div class="col-1 mb-1 mt-1">
                                    <i class="fa-2x ${data.details[i].isOwner ? "ri-vip-crown-2-fill" : "ri-user-fill"}"></i>
                        </div>
                        <div class="col-2">
                        ...
                        <div class="col-2 ${data.details[i].paid ? "bg-paid" : "bg-unpaid"}">
                            <h7 class="bold-text">${data.details[i].paid ? "PAID" : "UNPAID"}</h7>
                        </div>
